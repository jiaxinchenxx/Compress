...
public class BitReader {
	...
	public boolean next()throws Exception{
        ...
        int bit = current&0x80;  //Get the First bit of current with &10000000
        bit = bit>>7;  //move the bit you got to the last of the bytes
        //shift current to left for one bit
        current = current<<1;  //>>> shift and ignore the +/-
		if(bit==0)
			return false;
		return true;
	}
	...
	public static void main(String[] args) throws Exception{
		BitReader br = new BitReader("mmm.txz");
		while(br.hasNext()){
			if(br.next())
